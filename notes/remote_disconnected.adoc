...
Seeing is believing, so let's demonstrate.
...
Then run them (https://gist.github.com/cosmo-grant/5a3e64de2924f1ee359faf7c46dae790[full script]):
...
[source,text]
----
% uv run scripts/repro_remote_disconnected.py
=== PATCHED CLIENT ===
...
In 4.42, they https://github.com/braintree/braintree_python/commit/cde3faab72d93fae5f9c5a3f4a8f7841a3522716[changed] how they manage `requests.Session` objects.
Instead of creating a session per POST,
when you create a `BraintreeGateway`, it stores an `Http` internally, as `self.config._http_strategy`.
...
A sensible idea: faster and less wasteful, since you can re-use connections.

== Third ingredient: module-level gateway

It's standard, recommended practice in AWS Lambda to set up some resources (configuration, connections, and the like) at module-level,
so they're created once at INIT time,
and then re-used across warm invocations.
A sensible idea: faster and less wasteful, since you can re-use them.
...
and the remote drops the connection between ``urllib3``'s check and send.
