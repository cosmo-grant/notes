...
* preserved the old commit hashing behaviour under a new key `cache:key:files_commit`
...
* restored wildcard support, as in 209633
* but also prevented a file matching itself
...
But `cache:key:files_commit` still behaves as it did originally:
it calls `last_commit_id_for_path(path)`,
which is passed to Gitaly,
which interprets it as a pathspec.
